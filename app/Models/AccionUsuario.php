...
use App\Models\Expediente; // <-- 🔥 Importante: Asegúrate de tener este 'use'
...
class AccionUsuario extends Model
{
    ...
    public function getDescripcionHtmlAttribute(): string
    {
        ...
        switch ($this->tipo_accion) {
            ...
            // 🔥 ========================================================== 🔥
            // 🔥 NUEVO 'CASE' PARA MANEJAR LA DESCARGA DE PDF 🔥
            // 🔥 Integrado con tu estilo y lógica existentes.
            // 🔥 ========================================================== 🔥
            case 'DESCARGA_CONSTANCIA_PDF':
                // Usamos 'optional()' en cada paso para prevenir errores si algo fue eliminado.
                $expediente = optional(Expediente::find($this->referencia_id));
                $constancia = optional($expediente->constancia);
                $usuarioAfectado = optional($constancia->user);

                if ($expediente->numero_expediente) {
                    $numExp = $e($expediente->numero_expediente);
                    $numConst = $e($constancia->numero_constancia);
                    $nombreUsuario = $e($usuarioAfectado->name);

                    $html = "Descargó la constancia oficial (PDF) del expediente <code class=\"bg-gray-200 px-1 rounded font-semibold\">{$numExp}</code>.";
                    $html .= "<ul class=\"list-disc list-inside pl-2 text-xs mt-1\">";
                    $html .= "<li><strong>Constancia N°:</strong> {$numConst}</li>";
                    $html .= "<li><strong>Usuario de datos:</strong> {$nombreUsuario}</li>";
                    $html .= "</ul>";
                    return $html;
                }
                return 'Descargó una constancia (PDF) de un expediente que fue eliminado.';


            case 'DESCARGA_EXPEDIENTE':
                $expediente = Expediente::find($this->referencia_id);
                if ($expediente) {
                    $numExp = $e($expediente->numero_expediente);
                    return "Descargó el archivo Excel del expediente <code class=\"bg-gray-200 px-1 rounded font-semibold\">{$numExp}</code>.";
                }
                return 'Descargó un expediente (Excel) que fue eliminado posteriormente.';
            ...
            case 'EDICION_DATO_CRUDO':
                ...
                $nombreColumna = $e($this->nombre_columna_editada);
